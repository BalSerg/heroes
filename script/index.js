const filesSelfData1 = []; // Массив для хранения выбранных файлов 12
    ...
    battles = [
        {
            ...
            content: [
                ...
                {
                    name: 'Белорусская операция',
                    text: '',
                    id: 151052
                },
                {
                    name: 'Берлинская наступательная операция',
                    text: '',
                    id: 154182
                },
                {
                ...
                {
                    name: 'Оборона Севастополя и битва за Крым',
                    text: '',
                    id: 154860
                },
                {
                ...
                {
                    name: 'Керченско-Феодосийская десантная операция',
                    text: '',
                    id: 1739723
                },
                {
...
window.addEventListener('load', () => {
    let
        ...
        arrListsValue = getArray('.js-value-list'), // Переменнвя для значения списка
        elListHidden = getArray('.js-value-list-hidden'),// Невидимый элемент, // Вспомогательная переменная для значения списка
        ...
        arrayInputFields = getArray('.js-input-field'),

        arrayButtonsCallModalManage,
        arrayModalManageLink,
        arrayLayers,
        arrayDeleteButtons,
        arrayLinks,
        isHasLinks = false,
    ...
    function createLinks(paramName, paramValue) {
        return `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                ...
                <input type="hidden" class="js-ref" name="ref${paramName}" value="${paramValue}"/>
                <a href="${paramValue}" target="_blank" class="form-links__item-text js-link-url" data-current="0">${paramValue}</a>
    ...
    function forCreateLinks(selectors) {
        for(counter_i; counter_i<= countLinks; counter_i++) {
            if(window.localStorage.getItem(`ref${counter_i}`)) {
                let url = window.localStorage.getItem(`ref${counter_i}`);
                let div = document.createElement('DIV');
                div.classList.add('form-links__item');
                div.classList.add('js-link');
                div.innerHTML = createLinks(counter_i, url, '');
                getElement(selectors).append(div);
            }
        }
    }
    function openModalManage() {
        arrayButtonsCallModalManage = getArray(`.js-call-popup-manage-links`);
        arrayLayers = getArray(`.js-layer`);
        arrayModalManageLink = getArray(`.js-popup-manage-links`);
        arrayButtonsCallModalManage.forEach((item, index) => {
            item.addEventListener('click', () => {
                if(!arrayModalManageLink[index].classList.contains('active')){
                    arrayModalManageLink[index].classList.add('active');
                }
            });
        })
        arrayLayers.forEach((item, index) => {
            item.addEventListener('click', () => {
                arrayModalManageLink[index].classList.remove('active');
            });
        })
    }
    let countSaveLink = 0;
    ...
    //Функция проверки заполнения раздела формы для данных из localStorage
    function _checkIsCompleteSectionLS(elemSection, elemForSection) {
        let arrayBlocksLS = Array.from(elemSection.querySelectorAll('.js-block'));
        let flag = arrayBlocksLS.every((item) => {
            return item.classList.contains('is-complete');
        });
        if(flag) {
            elemSection.classList.add('is-complete');
            elemForSection.classList.add('is-complete');
        }
        else {
            elemSection.classList.remove('is-complete');
            elemForSection.classList.remove('is-complete');
        }

    }
    function checkIsCompleteSectionLS() {
        _checkIsCompleteSectionLS(getElement('.js-state1'), getArray('.js-timeline-item')[0])
        _checkIsCompleteSectionLS(getElement('.js-state2'), getArray('.js-timeline-item')[1])
        _checkIsCompleteSectionLS(getElement('.js-state3'), getArray('.js-timeline-item')[2])
    }
    //---------
    arrListsValue.forEach((item, index) => {
        if(window.localStorage.getItem(item.name)) {
            item.value = window.localStorage.getItem(item.name);
        }
        if(window.localStorage.getItem(elListHidden[index].name)) {
            elListHidden[index].value = window.localStorage.getItem(elListHidden[index].name);
        }
    })
    arrayInputFields.forEach((item) => {
        if(window.localStorage.getItem(item.name)) {
            if(item.name !== 'photos1') {
                item.value = window.localStorage.getItem(item.name);
            }
            if(item.name === 'photos1') {
                elViewImg.setAttribute('src', window.localStorage.getItem(item.name));
    ...
    if(elForm.militaryOperations.value.length > 0) {
        isRequired2 = true;
    }
    if(window.localStorage.getItem(elRadioValue.name)) {
        arrayRadioButtonValue.forEach((item, index) => {
            if(window.localStorage.getItem(elRadioValue.name) === item.textContent) {
                arrayRadioButton[index].checked = true;
                elRadioChooseWrapper.style.height = window.localStorage.getItem('heightBlock')+'px';
                elRadioChooseWrapper.classList.add('active');
            }
        })
        getArray('.js-block')[4].classList.add('is-complete');
        checkIsCompleteBlockLS();
    }
    function delLink() {
        let el = this.parentElement.parentElement.parentElement.parentElement.parentElement,// это общий div для всех ссылок
            link = this.parentElement.parentElement.parentElement.parentElement, // это текущая ссылка
            refs = [];
        window.localStorage.removeItem((el.querySelector(`.js-ref`).name));
        link.remove();
        if(el.childNodes.length ) {
            (el.querySelectorAll(`.js-ref`)).forEach((item) => {
                refs.push(item);
            })
        }
        countSaveLink = refs.length;
        if(el.classList.contains('lin1')) {
            refs.forEach((item, index) => {
                item.removeAttribute('name');
                item.setAttribute('name', `ref${index + 1}`);
            })
            for(let i = 0; i < countSaveLink; i++) {
                window.localStorage.setItem(refs[i].name, refs[i].value);
            }
            if(window.localStorage.getItem(`ref${countSaveLink}`) === window.localStorage.getItem(`ref${countSaveLink + 1}`)) {
                window.localStorage.removeItem(`ref${countSaveLink + 1}`);
            }
        }
        if(el.classList.contains('lin2')) {
            refs.forEach((item, index) => {
                item.removeAttribute('name');
                item.setAttribute('name', `ref${index + 6}`);
            })
            for(let i = 0; i < countSaveLink; i++) {
                window.localStorage.setItem(refs[i].name, refs[i].value);
            }
            if(window.localStorage.getItem(`ref${countSaveLink + 5}`) === window.localStorage.getItem(`ref${countSaveLink + 5 + 1}`)) {
                window.localStorage.removeItem(`ref${countSaveLink + 5 + 1}`);
            }
        }
    ...
    function opend() {
        let modalManage = this.nextElementSibling,
            layer = modalManage.querySelector('.js-layer');
        if(!modalManage.classList.contains('active')) {
    ...
    function editLink() {
        let currentHrefValue = this.parentElement.parentElement.parentElement.previousElementSibling.getAttribute('href'),
            modalEdit = this.parentElement.parentElement.parentElement.parentElement.parentElement.nextElementSibling.nextElementSibling.nextElementSibling.nextElementSibling,
            modalEditWrapper = modalEdit.querySelector('.js-wrapper-edit-links'),
            editInput = modalEdit.querySelector('.js-edit-url'),
            changeButton = modalEdit.querySelector('.js-change-link');
        //Скрыть модалку управления ссылками с кнопками Удалить Добавить
        this.parentElement.parentElement.classList.remove('active');
        //Открыть модалку редактирования
        openModal(modalEdit, modalEditWrapper);
        editInput.value = currentHrefValue;
        changeButton.addEventListener('click', () => {
            currentHrefValue = editInput.value;
            this.parentElement.parentElement.parentElement.previousElementSibling.setAttribute('href', currentHrefValue);
            this.parentElement.parentElement.parentElement.previousElementSibling.textContent = currentHrefValue;
            this.parentElement.parentElement.parentElement.previousElementSibling.previousElementSibling.value = currentHrefValue;
            window.localStorage.setItem(this.parentElement.parentElement.parentElement.previousElementSibling.previousElementSibling.name, currentHrefValue)
        })
    }
    let countLinks,
        counter_i,
        arrayDeleteButtonsLS = [],
        arrayButtonsCallModalManageLS = [],
        arrayEditButtonsLS = [];
    if(window.localStorage.getItem('ref1') || window.localStorage.getItem('ref2') || window.localStorage.getItem('ref3') || window.localStorage.getItem('ref4') || window.localStorage.getItem('ref5')) {
        counter_i = 1;
        countLinks = 5;
        forCreateLinks('.js-state1 .js-form-links');

    }
    if(window.localStorage.getItem('ref6') || window.localStorage.getItem('ref7') || window.localStorage.getItem('ref8') || window.localStorage.getItem('ref9') || window.localStorage.getItem('ref10')) {
        countLinks = 10;
        counter_i = 6;
        forCreateLinks('.js-state2 .js-form-links');
    }
    if(window.localStorage.getItem('isHasLinks') === 'true') {
        arrayButtonsCallModalManageLS = getArray(`.js-call-popup-manage-links`);
        arrayButtonsCallModalManageLS.forEach((item) => {
            item.addEventListener('click', opend);
        })
        arrayDeleteButtonsLS = getArray('.js-delete-link');
        arrayDeleteButtonsLS.forEach((item, index) => {
            item.addEventListener('click', delLink);
        })
        arrayEditButtonsLS = getArray('.js-edit-link');
        arrayEditButtonsLS.forEach((item) => {
            item.addEventListener('click', editLink)
        })
    }
    arrayFormLinks.forEach((item, index) => {
        if(item.childElementCount >= 5) {
            arrayButtonCallModalAddLinks[index].setAttribute('disabled','disabled');
        }
    })
    let arrayBattleFromLS = [];
    let arrayAwardFromLS = [],
        arrayAwardNameFromLS = [];
    if(window.localStorage.getItem('isHasBattle') === 'true') {
        function delBattle() {
            let elBlockWithLinks = getElement('.js-has-battle .js-form-choose'),
                arrayLinksBattle = getArray('.js-has-battle .js-chosen'),
                name = this.parentElement.previousElementSibling.previousElementSibling.name,
                currentIndex;
            currentIndex = arrayLinksBattle.indexOf(this.parentElement.parentElement);
            arrayLinksBattle.splice(currentIndex, 1);
            window.localStorage.removeItem(name);
            window.localStorage.removeItem(`battleUrl${currentIndex + 1}`);
            this.parentElement.parentElement.remove();
            arrayChosenBattleValue.splice(currentIndex, 1);
            let deletedValue = arrayChosenBattleValue.splice(currentIndex, 1)[0];
            arrayChooseItemNameBattle.splice(currentIndex, 1);
            let countLink = elBlockWithLinks.childElementCount;
            for(let i = 0; i < countLink; i++) {
                arrayLinksBattle[i].querySelector('.js-chosen-value').name = `battle${i + 1}`;
                window.localStorage.setItem(arrayLinksBattle[i].querySelector('.js-chosen-value').name, arrayLinksBattle[i].querySelector('.js-chosen-value').value);
                window.localStorage.setItem(`battleUrl${i + 1}`, arrayLinksBattle[i].querySelector('.js-chosen-value').nextElementSibling.textContent);
                if(Number(window.localStorage.getItem(`battle${i + 1}`)) === Number(window.localStorage.getItem(`battle${i + 2}`))) {
                    window.localStorage.removeItem(`battle${i + 2}`);
                    window.localStorage.removeItem(`battleUrl${i + 2}`);
                }
            }
            if(countLink < 1) {
                window.localStorage.removeItem('isHasBattle');
            }
        }
        let counterLink = 100;
        let elDiv = document.querySelector('.js-form-choose');
        if(elDiv.parentElement.classList.contains('js-has-battle')) {
            for(let i = 1; i <= counterLink; i++) {
                if(window.localStorage.getItem(`battle${i}`)) {
                    arrayChosenBattleValue.push(window.localStorage.getItem(`battle${i}`));
                    arrayChooseItemNameBattle.push(window.localStorage.getItem(`battleUrl${i}`));
                    let nameOfVarBattle = `battleUrl${i}`;
                    let valUrl = `https://ru.ruwiki.ru/wiki/${window.localStorage.getItem(nameOfVarBattle)}`;
                    let valText = window.localStorage.getItem(`battleUrl${i}`);
                    let val = window.localStorage.getItem(`battle${i}`);
                    let valName = `battle${i}`;
                    let div = document.createElement('div');
                    div.classList.add('form-links__item');
                    div.classList.add('js-chosen');
                    arrayBattleFromLS.push(val);
                    div.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                         <path d="M16.1078 17.8309L3.6158 7.06196L3.6158 3.61589L7.06187 3.61589L17.8308 16.1079M14.3848 19.554L16.9693 16.9694M19.5539 14.3849L16.9693 16.9694M16.9693 16.9694L20.4154 20.4155" stroke="#7659B1" stroke-width="1.5" stroke-linecap="round"></path>
                                         <circle cx="20.846" cy="20.8457" r="0.718315" transform="rotate(-45 20.846 20.8457)" stroke="#7659B1"></circle>
                                         <path d="M6.20154 16.1078L7.54766 14.5463L8.22073 13.7656M7.92458 17.8308L9.48608 16.4847L10.2668 15.8117M4.47851 14.3848L7.06306 16.9693M9.64761 19.5539L7.06306 16.9693M7.06306 16.9693L3.61699 20.4154M13.8247 7.1231L14.2783 6.7388L16.9705 3.6158L20.4166 3.6158L20.4166 7.06187L17.2936 9.75412L16.2615 10.7782" stroke="#7659B1" stroke-width="1.5" stroke-linecap="round"></path>
                                         <circle cx="3.18555" cy="20.8465" r="0.718315" transform="rotate(45 3.18555 20.8465)" stroke="#7659B1"></circle>
                                     </svg>
                                     <input type="hidden" class="js-chosen-value" name=${valName} value=${val}>
                                     <a href=${valUrl} target="_blank" class="form-links__item-text" data-current="0">${valText}</a>
                                     <div class="form-links__item-for-button">
                                         <div class="form-links__item-button js-chosen-delete">
                                             <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                 <path d="M18 6V16.2C18 17.8802 18 18.7202 17.673 19.362C17.3854 19.9265 16.9265 20.3854 16.362 20.673C15.7202 21 14.8802 21 13.2 21H10.8C9.11984 21 8.27976 21 7.63803 20.673C7.07354 20.3854 6.6146 19.9265 6.32698 19.362C6 18.7202 6 17.8802 6 16.2L6 6M4 6L20 6M16 6L15.7294 5.18807C15.4671 4.40125 15.3359 4.00784 15.0927 3.71698C14.8779 3.46013 14.6021 3.26132 14.2905 3.13878C13.9376 3 13.523 3 12.6936 3H11.3064C10.477 3 10.0624 3 9.70951 3.13878C9.39792 3.26132 9.12208 3.46013 8.90729 3.71698C8.66405 4.00784 8.53292 4.40125 8.27064 5.18807L8 6" stroke="#1E1E1E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                             </svg>
                                         </div>
                                     </div>`;
                    elDiv.append(div);
            ...
            let delButtons = getArray('.js-has-battle .js-chosen-delete');
            delButtons.forEach((item) => {
                item.addEventListener('click', delBattle);
            })
        }
    }
    if(window.localStorage.getItem('isHasAward') === 'true') {
        getElement('.js-has-awards').classList.add('is-complete');
        function delAward() {
            let elBlockWithLinks = getElement('.js-has-awards .js-form-choose'),
                arrayLinksAward = getArray('.js-has-awards .js-chosen'),
                name = this.parentElement.previousElementSibling.previousElementSibling.name,
                currentIndex;
            currentIndex = arrayLinksAward.indexOf(this.parentElement.parentElement);
            arrayLinksAward.splice(currentIndex, 1);
            window.localStorage.removeItem(name);
            window.localStorage.removeItem(`awardUrl${currentIndex + 1}`);
            this.parentElement.parentElement.remove();
            arrayChosenAwardValue.splice(currentIndex, 1);
            let deletedValue = arrayChosenAwardValue.splice(currentIndex, 1)[0];
            arrayChooseItemNameAward.splice(currentIndex, 1);
            let countLink = elBlockWithLinks.childElementCount;
            for(let i = 0; i < countLink; i++) {
                arrayLinksAward[i].querySelector('.js-chosen-value').name = `award${i + 1}`;
                window.localStorage.setItem(arrayLinksAward[i].querySelector('.js-chosen-value').name, arrayLinksAward[i].querySelector('.js-chosen-value').value);
                window.localStorage.setItem(`awardUrl${i + 1}`, arrayLinksAward[i].querySelector('.js-chosen-value').nextElementSibling.textContent);
                if(Number(window.localStorage.getItem(`award${i + 1}`)) === Number(window.localStorage.getItem(`award${i + 2}`))) {
                    window.localStorage.removeItem(`award${i + 2}`);
                    window.localStorage.removeItem(`awardUrl${i + 2}`);
                }
            }
            if(countLink < 1) {
                window.localStorage.removeItem('isHasAward');
                getElement('.js-has-awards').classList.remove('is-complete');
                checkIsCompleteBlockLS();
                checkIsCompleteSectionLS();
            }
        }
        let counterLink = 100;
        let elDiv = document.querySelector('.js-has-awards .js-form-choose');
        if(elDiv.parentElement.classList.contains('js-has-awards')) {
            for(let i = 1; i <= counterLink; i++) {
                if(window.localStorage.getItem(`award${i}`)) {
                    arrayChosenAwardValue.push(window.localStorage.getItem(`award${i}`));
                    arrayChooseItemNameAward.push(window.localStorage.getItem(`awardUrl${i}`));
                    let nameOfVarAward = `awardUrl${i}`;
                    let valUrl = `https://ru.ruwiki.ru/wiki/${window.localStorage.getItem(nameOfVarAward)}`;
                    let valText = window.localStorage.getItem(`awardUrl${i}`);
                    let val = window.localStorage.getItem(`award${i}`);
                    let valName = `award${i}`;
                    let div = document.createElement('div');
                    div.classList.add('form-links__item');
                    div.classList.add('js-chosen');
                    arrayAwardFromLS.push(val);
                    arrayAwardNameFromLS.push(valText);
                    div.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                         <path d="M16.1078 17.8309L3.6158 7.06196L3.6158 3.61589L7.06187 3.61589L17.8308 16.1079M14.3848 19.554L16.9693 16.9694M19.5539 14.3849L16.9693 16.9694M16.9693 16.9694L20.4154 20.4155" stroke="#7659B1" stroke-width="1.5" stroke-linecap="round"></path>
                                         <circle cx="20.846" cy="20.8457" r="0.718315" transform="rotate(-45 20.846 20.8457)" stroke="#7659B1"></circle>
                                         <path d="M6.20154 16.1078L7.54766 14.5463L8.22073 13.7656M7.92458 17.8308L9.48608 16.4847L10.2668 15.8117M4.47851 14.3848L7.06306 16.9693M9.64761 19.5539L7.06306 16.9693M7.06306 16.9693L3.61699 20.4154M13.8247 7.1231L14.2783 6.7388L16.9705 3.6158L20.4166 3.6158L20.4166 7.06187L17.2936 9.75412L16.2615 10.7782" stroke="#7659B1" stroke-width="1.5" stroke-linecap="round"></path>
            ...
            let delButtons = getArray('.js-has-awards .js-chosen-delete');
            delButtons.forEach((item) => {
                item.addEventListener('click', delAward);
            })
        }
        checkIsCompleteBlockLS();
        checkIsCompleteSectionLS();
    }
    if(window.localStorage.getItem('isLKComplete')) {
        getElement('.js-state1 .js-block').classList.add('is-complete');
        checkIsCompleteBlockLS();
    }
    let jsForBlock = getArray('.is-no-mobile .js-for-block');
    let jsForBlock1 = getArray('.js-modal-popup-timeline .js-for-block');
    let blocksWithTextarea = [];
    let forBlocksWithTextarea = [];
    let forBlocksWithTextarea1 = [];
    getArray('.js-textarea').forEach((item ,index) => {
        if(item.value === window.localStorage.getItem(item.name) && item.value.length > 1) {
            item.parentElement.parentElement.classList.add('is-complete');
            blocksWithTextarea.push(item.parentElement.parentElement);
            forBlocksWithTextarea.push(jsForBlock[1]);
            forBlocksWithTextarea.push(jsForBlock[3]);
            forBlocksWithTextarea.push(jsForBlock[6]);
            forBlocksWithTextarea.push(jsForBlock[7]);
            forBlocksWithTextarea.push(jsForBlock[8]);
            forBlocksWithTextarea1.push(jsForBlock1[1]);
            forBlocksWithTextarea1.push(jsForBlock1[3]);
            forBlocksWithTextarea1.push(jsForBlock1[6]);
            forBlocksWithTextarea1.push(jsForBlock1[7]);
            forBlocksWithTextarea1.push(jsForBlock1[8]);
        }
        blocksWithTextarea.forEach((item, index) => {
            if(item.classList.contains('is-complete')) {
                forBlocksWithTextarea[index].classList.add('is-complete');
            }
            else {
                forBlocksWithTextarea[index].classList.remove('is-complete');
            }
            if (item.classList.contains('is-complete')) {
                forBlocksWithTextarea1[index].classList.add('is-complete');
            }
            else {
                forBlocksWithTextarea1[index].classList.remove('is-complete');
            }
        })
        checkIsCompleteBlockLS();
    });
    if(window.localStorage.getItem('isEndLife')) {
        getArray('.js-block')[getArray('.js-block').length - 1].classList.add('is-complete');
        checkIsCompleteBlockLS();
    }
    if(window.localStorage.getItem('recruitPlace')) {
        getArray('.js-block')[2].classList.add('is-complete');
        checkIsCompleteBlockLS();
    }
    checkIsCompleteSectionLS();

    //Функция очистки полей и удаления всех значений
    function clearField() {
        arrayInputFields.forEach((item) => {
            item.value = '';
        })
        arrayFormLinks.forEach((item) => {
            item.textContent = '';
        })
        elListHidden.forEach((item) => {
            item.value = '';
        });
        elViewImg.src = '';
        elPlaceImg.classList.remove('active');
        getArray('.js-month').forEach((item) => {
            item.value = '';
        })
        getArray('.js-form-choose').forEach((item) => {
            item.textContent = '';
        })
        isRequired1 = false;
        isPublish = false;
        ...
        countBattle = 0;
        countAward = 0;
        elRadioChooseWrapper.removeAttribute('style');
        elRadioChooseWrapper.classList.remove('active');
        arrayRadioButton.forEach((item) => {
            item.checked = false;
        })
        ...
        getArray('.js-for-block').forEach((item) => {
            item.classList.remove('is-complete');
        })
        getArray('.js-timeline-item').forEach((item) => {
            item.classList.remove('is-complete');
        })
        arrayBattleFromLS = [];
        arrayAwardFromLS = [];
    ...
    function set_msg(msg) {
        ...
        if (!msg.data.user_info.result) { // юзер не авторизован
            ...
            currentLogin = msg.data.user_info.name;
            if(msg.data.user_info.name !== window.localStorage.getItem('login')) {
                clearField();
            }
        }
    ...
    elButtonLogin.addEventListener('click', () => {
        ...
        if(elLogin.value !== window.localStorage.getItem('login')) {
            clearField();
        }
        let data_send = {};
    ...
    //-------
    //Функция проверки заполенения блоков для данных из localstorage
    function checkIsCompleteBlockLS() {
        arrayBlocks = getArray('.js-block');
        arrayForBlocks = getArray('.is-no-mobile .js-for-block');
        arrayForBlocks1 = getArray('.js-modal-popup-timeline .js-for-block');
        arrayBlocks.forEach((item, index) => {
            if (item.classList.contains('is-complete')) {
                arrayForBlocks[index].classList.add('is-complete');
            }
    ...
    function permissionAlphabet(classOfInputs, setOfSymbols) {
        ...
        getArray(classOfInputs).forEach((item) => {
            item.addEventListener('input', () => {
                ...
                let enteredInfo = Array.from(item.value);
                badSymbols =  enteredInfo.filter(value => !setOfSymbols.includes(value));
                if(badSymbols.length > 0) {
                    enteredInfo.forEach((etem) => {
                        if(badSymbols.includes(etem)) {
                            item.value = item.value.replace(etem, '')
                        }
                    })
                }
            })
        ...
        arrayBlocks.forEach((item) => {
            ...
            arrayInputs.forEach((etem) => {
                etem.addEventListener('input', () => {
                    ...
                    if(elForm.lastName.value.length >= 2 && elForm.firstName.value.length >= 2 && elForm.middleName.value.length >= 2 && elForm.birthDate.value.length >= 1 && elForm.birthMonth.value.length >= 1 && elForm.birthYear.value.length >= 4 && elForm.birthPlace.value.length >= 2) {
                        item.classList.add('is-complete');
                        window.localStorage.setItem('isLKComplete', 'true');
                    }
                    else {
                        item.classList.remove('is-complete');
                        window.localStorage.removeItem('isLKComplete');
                    }
    ...
    arrayInputFields.forEach((item) => {
        item.addEventListener('input', function () {
            if(item.name !== 'photos1') {
                ...
                window.localStorage.setItem('login', currentLogin);
            }
        })
    })
        ...
        arrDates.forEach((item, index) => {
            item.addEventListener('input', () => {
                ...
                if(item.classList.contains('js-date-endLife')) {
                    ...
                    if(elForm.endMonth.value.length > 0 && elForm.endYear.value.length > 3 && elForm.endDate.value.length > 0) {
                        window.localStorage.setItem('isEndLife', 'true');
                    }
                    else {
                        window.localStorage.removeItem('isEndLife');
                    }
                }
            ...
            item.addEventListener('blur', () => {
                ...
                if(!elForm.birthMonth.value.length && elForm.birthDate.value) {
            ...
            arrList.forEach((etem, endex) => {
                etem.addEventListener('click', (e) => {
                    ...
                    if(etem.value) {
                        ...
                        if(inputsYear[index].value.length >= 4) {
                            isRequired1 = true;
                        }
                        else {
                            isRequired1 = false;
                    ...
                    if(arrListsValue[index].classList.contains('js-month-endLife')) {
                        ...
                        if(elForm.endDate.value.length > 0 && elForm.endYear.value.length > 3 && elForm.endMonth.value.length > 0) {
                            window.localStorage.setItem('isEndLife', 'true');
                        }
                        else {
                            window.localStorage.removeItem('isEndLife');
                    ...
                    window.localStorage.setItem(arrListsValue[index].name, arrListsValue[index].value);
                    window.localStorage.setItem(elListHidden[index].name, elListHidden[index].value);
                    window.localStorage.setItem(inputsDate[index].name, inputsDate[index].value);
    ...
    function checkYear() {
        arrayBlocks.forEach((item) => {
            let arrayYear = item.querySelectorAll('.js-year'),
                ...
                elMonthHidden = item.querySelector('.js-value-list-hidden'),
                numberMonth;
            arrayYear.forEach((etem, endex) => {
                etem.addEventListener('input', () => {
                    ...
                    if(etem.classList.contains('js-year-endLife')) {
                        if(elForm.endDate.value.length > 0 && elForm.endMonth.value.length > 0 && elForm.endYear.value.length >= 4) {
                            window.localStorage.setItem('isEndLife', 'true');
                        }
                        else {
                            window.localStorage.removeItem('isEndLife');
                    ...
                    window.localStorage.setItem(etem.name, etem.value);
                    window.localStorage.setItem(elDates.name, elDates.value);
                    window.localStorage.setItem(elMonth.name, elMonth.value);
                    window.localStorage.setItem(elMonthHidden.name, elMonthHidden.value);
                })
            })
        })
    }
    ...
    elUploadButton.addEventListener('click', () => {
        ...
        window.localStorage.setItem('photos1', elFileLoad.src);
    ...
    arrayButtonCallModalAddLinks.forEach((item, index) => {
        item.addEventListener('click', (e) => {
            ...
            let start = String(e.target.parentElement.classList).indexOf('js-has-link'),
                testingClass = String(e.target.parentElement.classList).substr(start);
            switch (testingClass) {
                case 'js-has-link1' :
                    counterLink = e.target.previousElementSibling.childElementCount;
                    break;
                case 'js-has-link2' :
                    counterLink = e.target.previousElementSibling.childElementCount;
                    break;
        ...
        arrayButtonsAddLink[index].addEventListener('click', (e) => {
            ...
            div.classList.add('form-links__item');
            div.classList.add('js-link');
            if(arrayFormLinks[index].classList.contains('lin1')) {
                div.innerHTML = createLinks(counterLink, elInputUrl.value);
                window.localStorage.setItem(`ref${counterLink}`, elInputUrl.value);
            }
            if(arrayFormLinks[index].classList.contains('lin2')) {
                a = 5;
                div.innerHTML = createLinks(counterLink + a, elInputUrl.value);
                window.localStorage.setItem(`ref${counterLink + a}`, elInputUrl.value);
            }
            arrayFormLinks[index].append(div);
            window.localStorage.setItem('isHasLinks', 'true');

            arrayLinks = getArray('.js-link');// Массив блоков со ссылкой внутри
            let arrayLinkUrl = [], // Массив непосредственно ссылок внтури блока
                //Массив кнопок редактировать ссылку
                arrayEditButtons = getArray('.js-edit-link');

            // Массив модалок управления ссыоками , внутри кнопки Удалить и Редактировать
            ...
            //Удаяем обработчики показа модкалки Удалить\Редактировать, удаления ссылки и реадктирования ссылки
            arrayButtonsCallModalManageLS = getArray('.js-call-popup-manage-links');
            for(let i of arrayButtonsCallModalManageLS) {
                i.removeEventListener('click', opend);
            }
            arrayDeleteButtonsLS = getArray('.js-delete-link');
            for(let i of arrayDeleteButtonsLS) {
                i.removeEventListener('click', delLink);
            }
            arrayEditButtonsLS = getArray('.js-edit-link');
            for(let i of arrayEditButtonsLS) {
                i.removeEventListener('click', editLink);
            }

            //обработчик показа модалки Удалить\Редактировать
            arrayButtonsCallModalManage.forEach((etem) => {
                etem.addEventListener('click', opend);
            })
            ...
            //обработчик кнопок Удалить ссыклу
            arrayDeleteButtons.forEach((etem) => {
                etem.addEventListener('click', delLink);
            })
            //обработчик кнопок Редактировать ссыклу
            arrayEditButtons.forEach((otem) => {
                otem.addEventListener('click', editLink);
            })
        ...
        arrayButtonChangeLink[index].addEventListener('click', function () {
            let start,
                elInputUrl = this.parentElement.previousElementSibling;
            if(elInputUrl.value.indexOf('//') > 0) { // если есть // то змаеняем все что до // на https://
                start = elInputUrl.value.indexOf('//') + 2;
                elInputUrl.value = 'https://' + elInputUrl.value.substr(start);
            }
            ...
            if(arrayModalEdit[index].querySelector('.js-edit-url').value.length > 0) {
                ...
                for(let i of arrayLinkUrl) {
                    if(i.dataset.current === '1') {
                        ...
                        window.localStorage.setItem(i.previousElementSibling.name, i.textContent);
                    }
                }
            }
        })
    ...
    getArray('.js-block').forEach((item) => {
        ...
        if(elButtonsCallModalChoose) {
            ...
            elButtonsCallModalChoose.addEventListener('click', (e) => {
                if(e.target.classList.contains('is-battle')) {
                    counterClick1++
                }
                if(!e.target.classList.contains('is-battle')) {
                    counterClick2++;
                ...
                if(item.classList.contains('js-has-awards')) {
                    ...
                    arrayFromLS = arrayAwardFromLS;
                }

                //---
                if(counterClick1 <=1 || counterClick2 <= 1) {// Услвоие по которому содержимое модалки создается только 1 раз
                    arrayData.forEach((etem) => {
                        ...
                        etem.content.forEach((otem, ondex) => {
                            let elLabelItem = document.createElement('LABEL'),
                            ...
                            arrayFromLS.forEach((item, index) => {
                                if(Number(item) === otem.id && arrayAwardNameFromLS[index] === otem.name) {
                                    elLabelItem.querySelector('.js-choose').checked = true;
                                    elButtonsAddChosen.classList.remove('disabled');
                                    elButtonsAddChosen.removeAttribute('disabled');
                                }
                            })
                            for(let i of arrayFromLS) {
                                /*if(Number(i) === otem.id) {
                                    console.log(3001);
                                    elLabelItem.querySelector('.js-choose').checked = true;
                                    elButtonsAddChosen.classList.remove('disabled');
            ...
            elButtonsCallModalChoose.nextElementSibling.querySelector('.js-button-add-choose').addEventListener('click', () => {
                ...
                if(item.classList.contains('js-has-battle')) {
                    arrayChosenBattleValue.forEach((etem, endex) => {
                        ...
                        elChosen.append(div);
                        window.localStorage.setItem('isHasBattle', 'true');
                        window.localStorage.setItem(`battle${endex + 1}`,`${etem}`);
                        window.localStorage.setItem(`battleUrl${endex + 1}`, `${arrayChooseItemNameBattle[endex]}`);
                        if(Number(window.localStorage.getItem(`battle${endex + 1}`)) === Number(window.localStorage.getItem(`battle${endex + 2}`))) {
                            window.localStorage.removeItem(`battle${endex + 2}`);
                            window.localStorage.removeItem(`battleUrl${endex + 2}`);
                        }
                    });
                }
                if(item.classList.contains('js-has-awards')) {
                    arrayChosenAwardValue.forEach((etem, endex) => {
                        ...
                        elChosen.append(div);
                        window.localStorage.setItem('isHasAward', 'true');
                        window.localStorage.setItem(`award${endex + 1}`,`${etem}`);
                        window.localStorage.setItem(`awardUrl${endex + 1}`, `${arrayChooseItemNameAward[endex]}`);
                        if(Number(window.localStorage.getItem(`award${endex + 1}`)) === Number(window.localStorage.getItem(`award${endex + 2}`))) {
                            window.localStorage.removeItem(`award${endex + 2}`);
                            window.localStorage.removeItem(`awardUrl${endex + 2}`);
                        }
                    });
                    getElement('.js-has-awards').classList.add('is-complete');
                    checkIsCompleteBlock();
                ...
                arrayButtonsDeleteChosen.forEach((etem, endex) => {
                    etem.addEventListener('click', () => {
                        for(let i of arrayChooseItem) {
                            if(i.value === arrayChosenInput[endex].value) {// если значение Inputa в модалке равно значению inputa  из добавленных
                                ...
                                if(item.classList.contains('js-has-battle')){
                                    ...
                                    let deletedItem = Number(arrayChosenBattleValue.splice(startPos, 1));
                                    let deletedItem2 = String(arrayChooseItemNameBattle.splice(startPos, 1));
                                    Object.keys(localStorage).forEach(key => {
                                        if(deletedItem === Number(localStorage.getItem(key))) {
                                            localStorage.removeItem(key);
                                        }
                                        if(deletedItem2 === String(localStorage.getItem(key))) {
                                            localStorage.removeItem(key);
                                        }
                                    });
                        ...
                        if(item.classList.contains('js-has-battle')) {
                            countBattle = elChosen.childElementCount;
                            for(let i = 1; i <= countBattle; i++) {
                                arrayChosenInput[i].name = `battle${i}`;
                                arrayChosenInput[i].value = arrayChosenBattleValue[i-1];
                                arrayChosenInput[i].nextElementSibling.textContent = arrayChooseItemNameBattle[i-1];
                                window.localStorage.setItem(`battleUrl${i}`, arrayChosenInput[i].nextElementSibling.textContent);
                                window.localStorage.setItem(arrayChosenInput[i].name, arrayChosenInput[i].value);
                                if(window.localStorage.getItem(arrayChosenInput[i]) === window.localStorage.getItem(arrayChosenInput[i])) {
                                    window.localStorage.removeItem(`battle${i+1}`);
                                    window.localStorage.removeItem(`battleUrl${i+1}`);
                                }
                            }
                            if(item.querySelector('.js-form-choose').childElementCount < 1) {
                                window.localStorage.removeItem('isHasBattle');
                            }
                        }
                        if(item.classList.contains('js-has-awards')) {
                            countAward = elChosen.childElementCount;
                            for(let i = 1; i <= countAward; i++) {
                                arrayChosenInput[i].name = `award${i}`;
                                arrayChosenInput[i].value = arrayChosenAwardValue[i-1];
                                arrayChosenInput[i].nextElementSibling.textContent = arrayChooseItemNameAward[i-1];
                                window.localStorage.setItem(`awardUrl${i}`, arrayChosenInput[i].nextElementSibling.textContent);
                                window.localStorage.setItem(arrayChosenInput[i].name, arrayChosenInput[i].value);
                                if(window.localStorage.getItem(arrayChosenInput[i]) === window.localStorage.getItem(arrayChosenInput[i])) {
                                    window.localStorage.removeItem(`award${i+1}`);
                                    window.localStorage.removeItem(`awardUrl${i+1}`);
                                }
                            }
                            if(item.querySelector('.js-form-choose').childElementCount < 1) {
                                window.localStorage.removeItem('isHasAwards');
                            }
                        }
    ...
    function changeItem() {
        if(this.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.classList.contains('js-has-battle')) {
            if(this.checked) {
                ...
                window.localStorage.removeItem(`battle${startPos + 1}`);
                window.localStorage.removeItem(`battleUrl${startPos + 1}`);
            }
        }
        if(this.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.classList.contains('js-has-awards')) {
            if(this.checked) {
                ...
                window.localStorage.removeItem(`award${startPos + 1}`);
                window.localStorage.removeItem(`awardUrl${startPos + 1}`);
            }
        }
    ...
    arrayRadioButton.forEach((item, index) => {
        ...
        item.addEventListener('change', () => {
            ...
            elForm.endCombatPathLocation.value = '';
            window.localStorage.setItem(elRadioValue.name, elRadioValue.value);
            window.localStorage.setItem('heightBlock', elRadioChoose.offsetHeight);
        ...
        switch (numberSection) {
            case 1:
                if(isRequired1) {
                    ...
                    if(window.localStorage.getItem(elRadioValue.name) === 'Погиб' || window.localStorage.getItem(elRadioValue.name) === 'Пропал без вести') {
                        elNext.classList.add('is-hidden');
                        elPublish.classList.remove('is-hidden');
                        arrayTimelineItems[arrayTimelineItems.length - 1].classList.add('is-complete');
                    }
                    else {
                        elNext.classList.remove('is-hidden');
                        elPublish.classList.add('is-hidden');
                        arrayTimelineItems[arrayTimelineItems.length - 1].classList.remove('is-complete');
                    }
                    window.scrollTo({
                        top: 0,
                        behavior: 'smooth'
    ...
    function fn() {
        if(window.localStorage.getItem('endCombatPathStatus') === 'Пропал без вести' || window.localStorage.getItem('endCombatPathStatus') === 'Погиб') {
            isNotAlive = true
        }
        else {
            isNotAlive = false
